---
title: "Gibson: format strings on s390x"
date: 2022-07-25
category: pwn
tags: [format-string, rop, s390x, us-cyber-open]
subtitle: "A format-string leak and a BOF-driven ROP chain, made harder by an architecture nobody debugs for fun."
env: "s390x / manual submission, no remote"
tldr: "Leak with a format string, then either ROP via the BOF or overwrite a GOT entry."
toc: true
---
...
## s390x
The s390x is a 64-bit big-endian architecture developed by IBM. Most of the instructions can be found at [tachyonsoft.com/inst390m.htm](http://www.tachyonsoft.com/inst390m.htm). Here's a list of the registers that look very similar to Arm registers
...
Googling Ghidra S390X resulted in this Chinese CTF blog post: [Bytectf 2019 s390 Writeup](https://gange666.github.io/2019/09/09/Bytectf_2019_s390_Writeup). Except the line with Ghidra was not a setup guide, rather it was a note that none of the disassembling tools worked normally. Major disappointment, but that means we only have objdump. Thankfully the program seems pretty simple.
...
## BUGS
Usually, in x86 the stack pointer is decremented at the start of each function to make space for local variables. Notice how 1192 is subtracted from r15 (the stack pointer) at the beginning of the main function
...
Printf takes in a buffer instead of a format string. This means that our transformed input will be passed directly to printf. The transformation does not seem too fancy as there's a one-to-one mapping. This means it's either adding a value or XORing our input
...
<h3>Attempt 1</h3>
If this was x86 the exploit path would be relatively simple, use printf to leak values and then overwrite GOT. The main difference is that I couldn't find the magical one_gadget. This means that I'll have to resort to replacing something in the GOT with system. Printf is a great candidate as our input is directly passed, so if we input /bin/sh we should get a shell.
The first issue is that after printf the program will exit, so we need a way to loop the program. Luckily with our buffer overflow, we can overwrite the return address with main so it will loop forever.
...
However, we can do a 16-bit overwrite to just change the last 2 bits of the GOT entry for printf. This was a very bad idea as most of the time ASLR base caused the difference to leak into the next byte. We would need to do some brute forcing to get a good libc address. 
```bash
...
Most of the time the libc base was wrong or it segfaulted upon connection but running this in a loop eventually worked. After a couple more submissions I found out that my exploit never worked. I thought it was just luck but after some time I found out that most people used an exploit that worked almost 100% of the time. At this point, I decided to rewrite this exploit for it to be more stable. After the competition, I found out that this idea would work but I was leaking values incorrectly. The stack was always shifting, so the %p on my machine might not be the same on remote. 


**Attempt 2** 
Relying on ASLR was not a good exploit attempt. I didn't want to ROP as I was hoping to avoid learning the architecture, but this was inevitable. After examining various functions, it seems that many functions have this epilogue.
...
At this point, I had a super reliable exploit locally, but on remote, it didn't work again. At this point, there were a couple of hours of the CTF left and I was freaking out. After opening a ticket, I realized that my libc addresses were wrong. Nothing made sense until I got an IP to connect to and I checked the value at libc base. To my surprise, it was not "ELF"; instead, it was some massive chunk of bytes. At this point, I realized that using %p to leak stack values is not a good idea as the stack shifts a lot. Instead, I used the format string to leak the GOT. After this, my exploit worked!! 

{{< figure src="fig1.png" alt="s390x stack layout diagram" caption="fig 1 — stack layout used for the ROP chain" >}}
