---
title: "Holy Grail of ROP"
date: 2022-08-15
category: pwn
tags: [rop, ret2dlresolve, x86, battelle]
subtitle: "Stack pivot into a fake link map, ret2dlresolve your way to a self-dumped binary."
env: "x86-32 / no PIE / partial RELRO"
tldr: "Parse the freshly dumped binary with Binja to find the vuln, stack pivot, place a fake link map, ret2dlResolve."
toc: true
---

*The ROP God has tasked King Arthur with finding the function called "holy_grail". You must aid him on his quest! But be warned, the way is guarded by a text-based sorcerer who loves old British comedy movies, and just to make things harder, you're going to have to find "holy_grail" 3 times! ...Or was it 5 times? Use your pwning knowledge to answer the sorcerer's questions and ROP your way to the holy grail and bring this holy relic home for the glory of England!*
...
[debugmen.dev: Holy Grail writeup](https://debugmen.dev/ctf-writeup/2022/01/14/holygrail.html)
...
```
After a couple of calls there's an "end" function that reads in data and does a final strncmp.
```c
int32_t sub_8048ae0()
{
...
At this point, I didn't want to look through these functions but I assumed that at least one of these should have a buffer overflow. I noticed that there was a call to memset on the buffer on the stack, so if the read size was larger than that memset then that function should be vuln. I just bought Binary Ninja recently and decided to give it a try. I searched through all the functions that returned `int32_t` and then checked the params of memset and read. Indeed there is a vuln function. 
...
Running in GDB, we can confirm that indeed we do have a buffer overflow
```gdb 
──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── registers ────
$eax   : 0x1
$ebx   : 0x41414141 ("AAAA"?)
...
## Attempt 1: Ret2dlResolve

This exploit is what I think of when I hear ret2dlResolve and it involves the construction of a bunch of fake tables. 

When a function is called for the first time and lazy loading is used (partial-Relo), the plt function will jump to the address in the GOT. When the GOT is first created, it holds a pointer back into the plt which will call dl_resolve. 
```gdb
gef➤  x/4i 134513552
...
The first value that is pushed is the function's offset in the jmprel.
...
R_Info is divided into 2 more fields, R_TYPE and R_SYM. The last two bytes are R_TYPE, it's always 7 for GOT entries. R_SYM is a value offset in the SYMTAB.
...
There are a couple of fields here but the main one that we are interested in is st_name. It's the offset into the STRTAB.
...
So when we call dl_resolve, we just need to push the jmp_offset and construct a fake symtab, jmptab and strtab. One thing to note is that the SYMTAB offset must be aligned; just place the structures and check in gdb.
...
Here's what our fake structs look like in memory (I replaced the addresses with the names of symbols)
```gdb
$eax   : 0x29
$ebx   : 0x0
...
Checking GDB and using the rsi feature, it seems that our offset was used as an offset into something. Since our value was so large, it ended up going into unmapped memory.
```gdb 
...
More GDB shows that sometimes the random value would be null; in this case, the mov would be successful as 0 shifted it still 0.
```gdb 
...
Since we have control over the value that gets added to ecx, we just need to shift our tables a bit so that it points to those nulls.
...
After a lot of fiddling, I realized that this attempt for remote would never work, so I moved on to my next attempt idea. After researching my next methods, I came across this article that explains this issue. Turns out this was a hash map index but the author used the same solution as mine.
[ctf-wiki: ret2dlresolve, Hand Forged Stage 4](https://ctf--wiki-org.translate.goog/pwn/linux/user-mode/stackoverflow/x86/advanced-rop/ret2dlresolve/?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en-US#partial-relro_1)

After solving this challenge I also saw this writeup that used this method and figured out how to fix this error:
[MooseTheGoose's HolyGrailOfRop writeup](https://github.com/MooseTheGoose/ctf-writeups/tree/main/Battelle22/HolyGrailOfRop)

## Attempt 2 Fake STRTAB with Link Map
...
L_Info is a giant array of addresses and they can be [looked up here](https://docs.oracle.com/cd/E19683-01/816-1386/chapter6-42444/index.html). Most important ones include DT_STRTAB, DT_SYMTAB and DT_JMPREL.
...
This means that in our link map we only need an l_addr, and l_info with strtab, symtab and jmprel pointers. Outside of the link map, we need to construct the jmprel structure as well as the extra structs to point to our tables. 
...
One thing to note is that in the jmprel, r_offset seems to be a funny value. The reason is that this address plus the offset is written to with the resolved address shortly before dl_resolve_runtime finishes. I figured this out by setting memory watchpoints. 
...
{{< figure src="fig1.png" alt="link map layout diagram" caption="fig 1 — fake link map layout for ret2dlresolve" >}}

With this, we can find the exact offset from execve and `__libc_start_main` and pop a shell!

After getting a shell we will notice that there's a hint.txt; for some reason I could not cat this file, but using base64 I could read it.
...
Grabbing the ld file and looking through the functions we see the holy grail. We could either call this function by leaking ld, but I want to keep this a leakless exploit. We can just get a shell and then do `echo DONE>./log; exit` to fake this function.
...
{{< figure src="fig2.png" alt="dl-resolve relocation diagram" caption="fig 2 — the relocation entry the fake link map points at" >}}
...
Huge thank you to playoff-rondo for showing me this challenge as well as ~~pushing~~ encouraging me through the suffering.
Questions or feedback? Feel free to reach out sofire=bad#6525 on discord.

Sources:
- [ir0nstone: ret2dlresolve](https://ir0nstone.gitbook.io/notes/types/stack/ret2dlresolve)
- [gist.github.com/inaz2](https://gist.github.com/inaz2/fbff517fc639f69a4309f79506771849)
- [delcoding: ret2dl_resolve_x64](https://delcoding-github-io.translate.goog/2019/03/ret2dl_resolve_x64/?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en-US)
- [speakerdeck: blinkroot HITCON 2015 writeup](https://speakerdeck.com/dhavalkapil/blinkroot-hitcon-2015-writeup?slide=21)
- [docs.oracle.com: ELF sections chapter](https://docs.oracle.com/cd/E19683-01/816-1386/chapter6-42444/index.html)
